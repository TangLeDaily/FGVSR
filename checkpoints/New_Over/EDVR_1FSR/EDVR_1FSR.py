...
class EDVR(nn.Module):
    ...
    def forward(self, x):
        ...
        nbr_feat_l = [  # reference feature list
            feat_l1[:, 0, :, :, :].clone(),
            feat_l2[:, 0, :, :, :].clone(),
            feat_l3[:, 0, :, :, :].clone()
        ]
        aligned_feat = self.pcd_align(nbr_feat_l, ref_feat_l)
        # aligned_feat = torch.stack(aligned_feat, dim=1)  # (b, c, h, w)
        aligned_feat = aligned_feat.unsqueeze(1)
