...
class GRU(nn.Module):
    def __init__(self):
        ...
        self.conv_out = nn.Conv2d(
            in_channels=64, out_channels=64, kernel_size=1, stride=1)  # (hidden_size, out_size)
    def forward(self, x, h_t_1):
        if h_t_1 is None:
            h_t_1 = x
        h_t_1 = h_t_1.cuda().detach()
...
class SPM_32C(nn.Module):
    def __init__(self, input_size=64, hidden_size=32):
        super(SPM_32C, self).__init__()
    ...
    def forward(self, input_, prev_hidden, prev_cell):
        ...
        out_h = self.lrelu(self.out_conv(hidden))
        # cell = cell + self.lrelu(self.enhence(prev_cell))
        return out_h, hidden, cell
...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, pre_h):
        b, t, c, h, w = aligned_feat.size()
        # temporal attention
        embedding_ref = self.temporal_attn1(
            aligned_feat[:, 0, :, :, :].clone())
        embedding_ref, next_h = self.GRU(embedding_ref, pre_h)
...
class EDVR(nn.Module):
    ...
    def forward(self, x, last_h):
        ...
        if last_h is None:
            feat, next_h = self.fusion(aligned_feat, None)
        else:
            feat, next_h = self.fusion(aligned_feat, last_h)
