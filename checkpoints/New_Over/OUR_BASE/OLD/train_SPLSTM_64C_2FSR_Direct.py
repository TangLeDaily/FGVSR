...
parser.add_argument("--batchSize", type=int, default=48, help="training batch size")  # default 16
...
def get_yu(model):
    kk = torch.load("/home/tangle/code/FGVSR/checkpoints/SPLSTM_test/model_epoch_212_psnr_27.3702.pth", map_location='cpu')
...
def main():
    ...
    str = "None"
    if opt.model_mark == 0:
        str = "SPLSTM_64C_2FSR_Direct"
    else:
        str = "ERROR"
    if use_wandb:
        wandb.init(project="SPVSR_V1", name=str, entity="karledom")
    print(opt)
    ...
    # checkpoint = torch.load("/home/tangle/code/FGVSR/checkpoints/SPLSTM_64C_2FSR_Direct/model_epoch_312_psnr_27.3756.pth", map_location='cpu')
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):
        train(optimizer, model, criterion, epoch, training_data_loader, test_loader)
        psnr = test_train_set(model, test_loader, epoch)
...
def train(optimizer, model, criterion, epoch, train_dataloader, test_loader):
    global min_avr_loss
    global save_flag
    ...
    for iteration, batch in enumerate(train_dataloader):
        ...
        if (iteration+1) % 1000 == 0:
            psnrr = test_train_set(model,test_loader, epoch)
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/SPLSTM_64C_2FSR_Direct/"
    else:
...
def test_train_set(this_model, test_loader, epoch_num):
    print(" -- Start eval --")
    ...
    with torch.no_grad():
        ...
        print("--->This--SPLSTM_64C_2FSR_Direct--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
