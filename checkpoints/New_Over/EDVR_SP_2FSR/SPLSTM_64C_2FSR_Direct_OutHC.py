...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, pre_h, pre_c):
        ...
        embedding_ref, next_h, next_c = self.LSTM(embedding_ref, pre_h, pre_c)
        out_h = next_h
        out_c = next_c
        ...
        return feat, next_h, next_c, out_start, out_h, out_c, after_dot
...
class EFVSR(nn.Module):
    ...
    def forward(self, x, last_h, last_c):
        ...
        if last_h is None:
            feat, next_h, next_c, out_start, out_h, out_c, after_dot = self.fusion(aligned_feat, None, None)
        else:
            feat, next_h, next_c, out_start, out_h, out_c, after_dot = self.fusion(aligned_feat, last_h, last_c)
        after_attn = feat
        out_start64 = out_start
        out_h64 = out_h
        out_c64 = out_c
        ...
        out_start = self.conv_last(out_start)
        out_h = self.conv_last(out_h)
        out_c = self.conv_last(out_c)

        after_attn = self.conv_last(after_attn)
        # after_dot = self.conv_last(after_dot)
