...
class feLSTM(nn.Module):
    ...
    def __init__(self, input_size=64, hidden_size=64):
        ...
        self.LSTM = LSTM()
        # self.conv = nn.Conv2d(input_size*2, input_size, 3, 3//2, 1)
        # self.RB = ResidualBlockNoBN()
        # self.relu = nn.LeakyReLU(negative_slope=0.1, inplace=True)

    def forward(self, input_, prev_hidden, prev_cell):
        # start = input_
        hidden, cell = self.LSTM(input_, prev_hidden, prev_cell)
        # out = self.relu(self.conv(torch.cat((hidden, start),dim=1)))
        # out = self.RB(out)
...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, pre_h, pre_c):
        ...
        embedding_ref, _ = self.attn(embedding_ref, outLfeat)
