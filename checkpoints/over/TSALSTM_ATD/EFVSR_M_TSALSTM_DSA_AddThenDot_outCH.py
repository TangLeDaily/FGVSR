...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, pre_h, pre_c):
        ...
        out_start = embedding_ref
        outLfeat, next_h, next_c = self.LSTM(embedding_ref, pre_h, pre_c)
        out_h = next_h
        out_c = next_c
        embedding_ref = self.fusion_LS(torch.cat((embedding_ref, outLfeat), dim=1))
        # embedding_ref = embedding_ref*outLfeat
        out_after = embedding_ref
        ...
        return feat, next_h, next_c, out_start, out_h, out_c, out_after
...
class EFVSR(nn.Module):
    ...
    def forward(self, x, last_h, last_c):
        ...
        if last_h is None:
            feat, next_h, next_c, out_start, out_h, out_c, out_after = self.fusion(aligned_feat, None, None)
        else:
            feat, next_h, next_c, out_start, out_h, out_c, out_after = self.fusion(aligned_feat, last_h, last_c)
        ...
        # out_start = self.reconstruction(out_start)
        # out_start = self.lrelu(self.pixel_shuffle(self.upconv1(out_start)))
        # out_start = self.lrelu(self.pixel_shuffle(self.upconv2(out_start)))
        # out_start = self.lrelu(self.conv_hr(out_start))
        out_start = torch.sigmoid(self.conv_last(out_start))

        # out_h = self.reconstruction(out_h)
        # out_h = self.lrelu(self.pixel_shuffle(self.upconv1(out_h)))
        # out_h = self.lrelu(self.pixel_shuffle(self.upconv2(out_h)))
        # out_h = self.lrelu(self.conv_hr(out_h))
        out_h = torch.sigmoid(self.conv_last(out_h))

        # out_c = self.reconstruction(out_c)
        # out_c = self.lrelu(self.pixel_shuffle(self.upconv1(out_c)))
        # out_c = self.lrelu(self.pixel_shuffle(self.upconv2(out_c)))
        # out_c = self.lrelu(self.conv_hr(out_c))
        out_c = torch.sigmoid(self.conv_last(out_c))


        # out_after = self.reconstruction(out_after)
        # out_after = self.lrelu(self.pixel_shuffle(self.upconv1(out_after)))
        # out_after = self.lrelu(self.pixel_shuffle(self.upconv2(out_after)))
        # out_after = self.lrelu(self.conv_hr(out_after))
        out_after = torch.sigmoid(self.conv_last(out_after))

        return out, next_h, next_c, out_start, out_h, out_c, out_after
