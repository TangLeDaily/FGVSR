...
parser.add_argument("--test_cuda", default=True, type=bool, help="use cuda?")
...
def main():
    ...
    # checkpoint = torch.load("checkpoints/DB_EDVR_L/model_epoch_888_psnr_27.1292.pth", map_location='cpu')
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/DB_EDVR_L/"
    else:
    ...
    with torch.no_grad():
        ...
        for iteration, batch in enumerate(test_loader, 1):
            ...
            if opt.test_cuda:
                input = input.cuda()
                target = target.cuda()
            out = model(input, gpu=False)
            psnr.update(calc_psnr(out, target), len(out))
        if use_wandb:
            wandb.log({'epoch': epoch_num, 'psnr': psnr.avg})
        print("--->This--DB_EDVR_L--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
