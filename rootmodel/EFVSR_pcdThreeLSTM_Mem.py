...
class PCDAlignment(nn.Module):
    ...
    def forward(self, pre_h, pre_c, nbr_feat_l, ref_feat_l, sav):
        # Pyramids
        for i in range(3, 0, -1):
            level = f'l{i}'
            ...
            if sav:
                feat, pre_c[i] = self.LSTM[level](feat, prev_hidden = pre_h[i], prev_cell = pre_c[i])
                pre_h[i] = feat
            else:
                next_c = None
                next_feat = None
...
class EFVSR(nn.Module):
    ...
    def forward(self, x, pre_h, pre_c):
        ...
        for i in range(t):
            ...
            if i == 0:
                feat, next_h, next_c = self.pcd_align(pre_h, pre_c, nbr_feat_l, ref_feat_l, sav=False)
                aligned_feat.append(feat)
            else:
                feat, next_h, next_c = self.pcd_align(pre_h, pre_c, nbr_feat_l, ref_feat_l, sav=True)
                aligned_feat.append(feat)
