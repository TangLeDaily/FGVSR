...
class PCDAlignment(nn.Module):
    ...
    def save_feature(self, feat_ID, i, feat):
    ...
    def forward(self, feat_ID, nbr_feat_l, ref_feat_l):
        # Pyramids
        ...
        for i in range(3, 0, -1):
            ...
            save_feat.append(feat + ref_feat_l[i - 1])
            # self.save_feature(feat_ID, i - 1, feat + ref_feat_l[i - 1])
            if i < 3:
        ...
        return feat, save_feat # 2-1-0，小中大
...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat):
        ...
        feat = feat * attn * 2 + attn_add + start
        return feat

class SPVSR_GNew(nn.Module):
    def __init__(self,
                 num_in_ch=3,
                 num_out_ch=3,
                 num_feat=64,
                 ...
                 with_tsa=True):
        super(SPVSR_GNew, self).__init__()
    ...
    def forward(self, x, feat_ID):
        ...
        feat_l2 = feat_l2.view(b, -1, h // 2, w // 2)
        feat_l3 = feat_l3.view(b, -1, h // 4, w // 4)
        ...
        if Nofirst:
            ...
            nbr_feat_l = [  # neighboring feature list
                pre_feat[0].clone().cuda(),
                pre_feat[1].clone().cuda(),
                pre_feat[2].clone().cuda()
            ]
            nbr_feat_l = nbr_feat_l
        else:
            nbr_feat_l = ref_feat_l

        aligned_feat = []
        a, _ = self.pcd_align(feat_ID, ref_feat_l, ref_feat_l)
        aligned_feat.append(a)
        a, a_feat = self.pcd_align(feat_ID, nbr_feat_l, ref_feat_l)
        aligned_feat.append(a)
        ...
        # extract features for each frame
        # L1
        out_feat_l1 = self.lrelu(self.conv_first(x.view(-1, c, h, w)))

        out_feat_l1 = self.feature_extraction(out_feat_l1)
        # L2
        out_feat_l2 = self.lrelu(self.conv_l2_1(out_feat_l1))
        out_feat_l2 = self.lrelu(self.conv_l2_2(out_feat_l2))
        # L3
        out_feat_l3 = self.lrelu(self.conv_l3_1(out_feat_l2))
        out_feat_l3 = self.lrelu(self.conv_l3_2(out_feat_l3))

        out_feat_l1 = out_feat_l1.view(b, -1, h, w)
        out_feat_l2 = out_feat_l2.view(b, -1, h // 2, w // 2)
        out_feat_l3 = out_feat_l3.view(b, -1, h // 4, w // 4)

        self.save_feature(feat_ID, 2, a_feat[0] + out_feat_l3)
        self.save_feature(feat_ID, 1, a_feat[1] + out_feat_l2)
        self.save_feature(feat_ID, 0, a_feat[2] + out_feat_l1)
