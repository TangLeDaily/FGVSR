...
class PCDAlignment(nn.Module):
    ...
    def forward(self, feat_ID, nbr_feat_l, ref_feat_l):
        ...
        for i in range(3, 0, -1):
            ...
            self.save_feature(feat_ID, i - 1, feat)
            if i < 3:
...
class fusion(nn.Module):
    ...
    def forward(self, ref_feat_l, nbr_feat_l, feat_ID):
        aligned_feat = []
        aligned_feat.append(self.pcd_align(feat_ID, ref_feat_l, ref_feat_l))
        aligned_feat.append(self.pcd_align(feat_ID, nbr_feat_l, ref_feat_l))
...
class GFeature(nn.Module):
    def __init__(self, RN_num=5, channel=64, kernel_size=3):
        ...
        self.conv_l2_1 = nn.Conv2d(channel, channel, 3, 2, 1)
        self.conv_l2_2 = nn.Conv2d(channel, channel, 3, 1, 1)
        self.conv_l3_1 = nn.Conv2d(channel, channel, 3, 2, 1)
        self.conv_l3_2 = nn.Conv2d(channel, channel, 3, 1, 1)

        m_body = [
            RB_NoBN(n_feats=channel, kernel_size=kernel_size, res_scale=1) for _ in range(RN_num)
        ]
        self.body = nn.Sequential(*m_body)
        # self.pcd_align = PCDAlignment(num_feat=channel, deformable_groups=8)
        self.fusion = fusion()# = TSAFusion(num_feat=channel, num_frame=2)
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)
    ...
    def forward(self, x, feat_ID):
        ...
        feat = self.fusion(ref_feat_l, nbr_feat_l, feat_ID)
