...
class TSAFusionNoP(nn.Module):
    def __init__(self, num_feat=64, num_frame=2):
        super(TSAFusionNoP, self).__init__()
        # temporal attention (before fusion conv)
        self.temporal_attn1 = nn.Conv2d(num_feat, num_feat, 3, 1, 1)
        self.temporal_attn2 = nn.Conv2d(num_feat, num_feat, 3, 1, 1)
        self.feat_fusion = nn.Conv2d(num_frame * num_feat, num_feat, 1, 1)
    ...
    def forward(self, aligned_feat):
        ...
        attn = self.lrelu(self.spatial_attn3(attn))
        attn = self.lrelu(self.spatial_attn4(attn))
        attn = self.upsample(attn)
        attn = self.spatial_attn5(attn)
        attn_add = self.spatial_attn_add2(
    ...
    def forward(self, x, feat_ID):
        ...
        if Nofirst:
            ...
            nbr_feat_l = [  # neighboring feature list
                self.attn(torch.stack((pre_feat[0].clone().cuda(), pre_feat[0].clone().cuda()), dim=1)),
                self.attn(torch.stack((pre_feat[1].clone().cuda(), pre_feat[1].clone().cuda()), dim=1)),
                # self.attn(torch.stack((pre_feat[2].clone().cuda(), pre_feat[2].clone().cuda()), dim=1)),
                # self.attn(pre_feat[0].clone().cuda()),
                # self.attn(pre_feat[1].clone().cuda()),
                self.attn(pre_feat[2].clone().cuda())
            ]
        ...
        aligned_feat = []
        aligned_feat.append(self.pcd_align(feat_ID, ref_feat_l, ref_feat_l, sav=False))
